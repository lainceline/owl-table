	...
	var OwlInput = React.createClass({
		...
		render: function () {
			...
			switch (props.column.type) {
				case 'text':
				case 'number':
					input = React.createElement("input", {className: "owl-input", type: props.column.type, onBlur: self.transmitSaveEvent, defaultValue: props.value, onKeyDown: self.keydown, onChange: self.inputDidChange});
					break;
				...
				case 'select_multiple':
					...
					if (props.column.type === 'select_multiple') {
						input = React.createElement("select", {onChange: self.inputDidChange, className: "swiftbox", multiple: true, defaultValue:  !props.value ? props.value : props.value.split("||")}, 
									optionList
								);
					} else {
						input = React.createElement("select", {onChange: self.inputDidChange, className: "swiftbox", defaultValue: props.value}, 
									optionList
								);
					}
					break;
				...
				case 'date':
					input =
						React.createElement("input", {className: "owl-input", defaultValue: props.value, 'data-date-format': "dd-M-yy", 'data-provide': "datepicker"});
					break;
				case 'time':
...
var OwlCell = React.createClass({
	...
	render: function () {
		...
		if (props.editable === true) {
			...
			td =
				React.createElement("td", {className: props.column.field, 'data-field': props.column.field, onClick: this.open}, 
					content
				);
		} else {
			td = React.createElement("td", {className: props.column.field, 'data-field': props.column.field, dangerouslySetInnerHTML: { __html: value}});
		}
	...
	render: function () {
		...
		var cells = props.columns.map(function (column, index) {
			...
			if (column.visible !== false) {
				return (
					React.createElement(OwlCell, {column: column, ref: ref, row: props.data, editable: editable, focusedCell: state.focusedCell, key: index, tableDidChange: props.tableDidChange})
				);
			}
		});

		return(
			React.createElement("tr", {className: props.className + " owl-row", key: props.key}, 
				cells
			)
		);
	}
...
var OwlTableReact = React.createClass({
	...
	render: function () {
		...
		var headers = props.columns.map(function (column, index) {
			...
			if (column.visible !== false) {
				return (
					React.createElement("th", {className: classes, key: index, 'data-field': column.field}, 
						column.title || 'None', 
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('partials/columnModal.html',
    '<div class="modal-header"><h3 class="modal-title">Customize Columns</h3></div><div class="modal-body"><li ng-repeat="column in columns"><a ng-click="toggleColumn(column)" ng-class="{strikethrough: !column.visible}">{{ column.title }}</a></li></div><div class="modal-footer"><button ng-click="ok()" class="btn btn-primary">Ok</button></div>');
}]);
})();
