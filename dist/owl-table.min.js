	...
	render: function () {
		...
		if (props.childColumns.length > 0) {
			_.forEach(props.childColumns, function (column) {
				...
				if (column.visible !== false) {
					cells.push(
						React.createElement(OwlCell, {column: column, ref: ref, row: props.data, isChild: props.isChild, editable: editable, focusedCell: state.focusedCell, key: cellCount, tableDidChange: props.tableDidChange})
					);
				}
...
var OwlTableReact = React.createClass({
	...
	render: function () {
		...
		var headers = props.columns.map(function (column, index) {
			...
			if (column.visible !== false) {
				return (
					React.createElement("th", {className: classes, id: id, key: headerCount, 'data-field': column.field}, 
						column.title || 'None', 
		...
		_.forEach(props.data, function (row, index) {
			...
			if (!_.isUndefined(children) && _.isArray(children)) {
				_.forEach(children, function (child, index) {
					...
					rowsWithChildren.push(
						React.createElement(OwlRow, {data: child, isChild: true, childColumns: props.childColumns, columns: props.columns, key: rowCount, open: self.state.openRows[index] || false, tableDidChange: self.tableDidChange})
					);
					rowCount++;
				});
			}
