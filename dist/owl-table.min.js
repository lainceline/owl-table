	...
	var OwlInput = React.createClass({
		...
		render: function () {
			...
			switch (props.column.type) {
				case 'text':
				case 'number':
					input = React.createElement("input", {type: props.column.type, defaultValue: props.value, onChange: this.changeHandler});
					break;
				case 'select':
...
var OwlCell = React.createClass({
	...
	render: function () {
		var props = this.props;
		var content = React.createElement("span", {className: "owl-cell-value-label"}, props.value);

		props.editable = props.column.editable || true;

		if (props.open === true) {
			content = React.createElement(OwlInput, {column: props.column, value: props.value});
		}
...
var OwlRow = React.createClass({
	...
	render: function () {
		...
		var cells = props.columns.map(function (column, index) {
			return (
				React.createElement(OwlCell, {column: column, value: props.data[column.field], open: state.open})
			);
		});
